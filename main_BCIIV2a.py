...
def bci4_2a():  # 通用模型模块, 不需要更改, 放在main中方便调试;
    dataset = 'BCI4_2A'
    data_path = "/root/autodl-tmp/LMDA-Code-main/bci2a_data/"
    ...
    test_cnt = mne_apply(lambda a: bandpass_cnt(a, low_cut_hz=4, high_cut_hz=38,
                         ...
                         test_cnt)
    # 提取原始数据和采样率
    train_data_raw = train_cnt.get_data()
    test_data_raw = test_cnt.get_data()
    sfreq = train_cnt.info['sfreq']

    # 调用更新后的函数
    train_data, train_label = extract_segment_trial(train_data_raw, train_events, sfreq)
    test_data, test_label = extract_segment_trial(test_data_raw, test_events, sfreq)
